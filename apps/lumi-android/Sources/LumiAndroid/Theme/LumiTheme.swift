...
//
// Ported from apps/lumi-ios/Lumi/LumiTheme.swift. UIKit-dependent variable
// font axis manipulation is iOS-only (#if !SKIP); on Android we fall back
// to the static NotoSerifDisplay-Light .ttf bundled in Resources/Fonts.
// iOS 26 .glassEffect calls are gated by #available; older iOS and Android
// both render the ZenGlass translucent surface fallback.
...
enum LumiTheme {
    ...
    /// On-surface: warm near-black (#292524)
    ...
    /// Noto Serif Display.
    /// - iOS: applies variable font axes (CTGR=100, wdth=100, wght=weight) on a UIFont,
    ///   so a single `NotoSerifDisplay-Regular.ttf` can render any weight.
    /// - Android (Skip): we ship the static `NotoSerifDisplay-Light.ttf` only, so the
    ///   weight parameter is ignored and we always return the Light cut.
    static func notoSerifDisplay(size: CGFloat, weight: CGFloat) -> Font {
        #if canImport(UIKit)
        if let base = UIFont(name: "NotoSerifDisplay-Regular", size: size) {
            let descriptor = base.fontDescriptor.addingAttributes([
                UIFontDescriptor.AttributeName(rawValue: "NSCTFontVariationAttribute"): [
                    0x77676874: weight, // wght
                    0x43544752: 100,    // CTGR (Display)
                    0x77647468: 100     // wdth
                ]
            ])
            return Font(UIFont(descriptor: descriptor, size: size))
        }
        // Display-Regular missing — fall through to the static Light cut.
        return .custom("NotoSerifDisplay-Light", size: size)
        #else
        // macOS host build / Skip Android — single static cut shipped in Resources/Fonts.
        return .custom("NotoSerifDisplay-Light", size: size)
        #endif
    }
...
struct AuroraBackground: View {
    var body: some View {
        ZStack {
            ...
            // Cherry blossom pink radial - top left
            RadialGradient(
            ...
            // Peach warm radial - bottom right
            RadialGradient(
...
// MARK: - Zen Glass Material
//
// iOS path: `.ultraThinMaterial` blur + 30% white overlay + 60% white border.
// Skip Android path: solid 30-40% white surface + same border + same shadow.
// Skip's SwiftUI shim does not implement `.ultraThinMaterial`, so the Material
// fill is gated; visual fidelity loss is intentional and acceptable for v1.
...
struct ZenGlass: ViewModifier {
    ...
    private var glassFill: some View {
        ...
        #else
        // No backdrop blur on Android — keep the cream base + warm border to
        // preserve the Lumi character without the iOS material lift.
        shape
