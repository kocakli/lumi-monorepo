...
struct PairingSuccessAnimation: View {
    ...
    @State var orbitAngle: Double = 0

    // Phase 2: Merging
    @State var orbitRadius: CGFloat = 100
    @State var mergeScale: CGFloat = 1.0
    @State var pulseScale: CGFloat = 0.0
    @State var pulseOpacity: Double = 0.0

    // Phase 3: Blossom burst + text
    @State var petalOffsets: [CGSize] = Array(repeating: .zero, count: 15)
    @State var petalOpacities: [Double] = Array(repeating: 0.0, count: 15)
    @State var textOpacity: Double = 0.0

    // Phase 4: Fade out
    @State var overlayOpacity: Double = 1.0
