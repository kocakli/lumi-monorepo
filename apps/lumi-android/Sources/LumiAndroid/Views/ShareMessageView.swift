...
struct ShareCardRenderable: View {
    let message: String
    let mood: String
...
// Skip Android stub: ShareLink/Intent.ACTION_SEND-based share will land
// when the share-sheet behaviour is exercised. For now keep the symbol
// available so call-sites compile; Android render path is TODO (Faz 11+).
struct ShareMessageView: View {
    @Environment(\.dismiss) var dismiss
    let message: String
    let mood: String
    ...
    var body: some View {
        VStack(spacing: 16) {
            Text("Sharing not yet wired on Android")
                .font(LumiTheme.body(14))
