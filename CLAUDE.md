...
**Build / version**: `MARKETING_VERSION` and `CURRENT_PROJECT_VERSION` are set in `project.yml` (currently 1.0.0 / build 21). All four targets (Lumi, LumiWidgetExtension, LumiWatch, LumiWatchComplication) carry the same version; bumping the build requires editing all four blocks in `project.yml` and re-running `xcodegen generate`.
...
**Current state**: Auth, Firestore, Functions, Messaging integrated via SPM. Pairing system, push notifications (with scheduled hourly job), account deactivation, ambient audio, and the watchOS companion all ship in build 21. Build 20 denormalized a `senderDisplayName` onto each paired message doc so paired messages always carry a sender attribution (Apple Guideline 1.1.6). Build 21 adds Guideline 1.2 (User-Generated Content) compliance: a first-launch `TermsAcceptanceView` gate with a zero-tolerance clause and links to `https://lumi.tease.tr/terms` and `https://lumi.tease.tr/privacy`; a top-right ellipsis "Block this sender" affordance on every swipe card that routes through the existing `reportMessage` → `processReports` pipeline; and a "Contact us" `mailto:` link near the bottom of Settings. Backend runs on Node 22 with 20 functions deployed in `europe-west1`. No tests. No linting or CI/CD configured.
